...
>>>>>>> 85a3338df5527ab7912311c89acb74d6e42f0318
=======
We first tried to rescale our model as proposed by Tien and Earn \textbf{CITE}. It seems, though, that their rescaled model equations are somewhat paradoxical. Looking at the dimensions of equation \eqref{eq:SIWR_susceptible}} there already seems to be some contradiction. The dimension of $\.{s}$,s,i and r is $km^{-2}$. The dimension of $\mu$, $\beta_{w}$ and $\beta_{i}$ is declared as $day^{-1}$. This shows that the dimensions on the left and right side of the equation cannot be the same. Because of this, we decided to let our susceptible, infectious and removed compartments have the dimension of percentages. As a consequence, we do not know what dimensions our coefficients have (e.g. $day^{-1}$ or $cells*ml^{-3}$). We will not be able to make the same conclusions as XXXX. Primarily, we will be able to make qualitative statements about the proportions of transmission caused by the local population versus the interdepartmental transmission.
>>>>>>> Latex files updated
...
\begin{empheq}[left=\empheqlbrace]{align}
\dfrac{ds}{dt} = - \lambda s                \label{eq:sir_susceptible} \\
\dfrac{dx}{dt} = \lambda s - \gamma x       \label{eq:sir_infectious} \\
\dfrac{dr}{dt} = \gamma x                   \label{eq:sir_removed}
\end{empheq}
